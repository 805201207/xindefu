...
$sql = "SELECT id,cre_time,content FROM news LIMIT ".($page-1)*$PageSize .",$PageSize";
if(!$sql){
    echo "取出不成功";
};
//把sql语句传送到数据库
...
echo "<tr><td>ID</td><td>时间</td><td>内容</td><td>编辑</td></tr>";
while($row = mysql_fetch_assoc($result)){
    echo "<tr>";
    echo "<td>{$row['id']}</td>";
    echo "<td>{$row['cre_time']}</td>";
    echo "<td>";
    echo substr($row['content'],0,30);
    echo "</td>";
    echo "<td>";
    ?>
    <a href="delete_list.php?id=<?php echo $row['id']?>">删除</a>
    <a href="updatanews.php?id=<?php echo $row['id']?>">修改</a>
