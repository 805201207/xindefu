...
$sql = "SELECT * FROM wujinbiao LIMIT ".($page-1)*$PageSize .",$PageSize";
if(!$sql){
    echo "取出不成功";
};
//把sql语句传送到数据库
...
echo "<tr><td>ID</td><td>时间</td><td>总类型</td><td>子类型</td><td>详情</td><td>编辑</td></tr>";
while($row = mysql_fetch_assoc($result)){
    echo "<tr>";
    echo "<td>{$row['id']}</td>";
    echo "<td>{$row['cre_time']}</td>";
    echo "<td>{$row['TotalModel']}</td>";
    ...
    echo "<td>";
    ?>
    <a href="wujin-delete.php?id=<?php echo $row['id']?>">删除</a>
...
//获取数据总数
$to_sql="SELECT COUNT(*)FROM wujinbiao";
