	...
	)
{
    ...
    else
    {
		//更换图片的名字
		$date=date('Ymdhis');
		$newName1=$date ."sp1" . '.' . $temp1[1];
		$newName2=$date ."sp2" . '.' . $temp2[2];
		$newName3=$date ."sp3" . '.' . $temp3[3];
		$newName4=$date ."sp4" . '.' . $temp4[4];
...
cre_time,type,URL1,URL2,URL3,URL4,Details) values ('$time',' $zhonleixin','$photo1','$photo2','$photo3','$photo4','$jiesou')";
						...
						if($que){
							echo "<script>alert('添加成功');location.href='http://localhost:8080/lena/xindefu/xindefu-du-h-du/member-fenlei-spyp.php'</script>";
						}else{
