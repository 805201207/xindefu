	...
	<body>
		...
		<form method="POST" action="" name="frm" enctype="multipart/form-data" onsubmit="return foo();">
		 ...
		 <select name="province" id="province">
			<option value="0">请选择非标定制类型</option>
			<option value="相关配件">相关配件</option>
			<option value="非标案例">非标案例</option>
		</select>
	...
	)
{
    ...
    else
    {
		//更换图片的名字
		$date=date('Ymdhis');
		$newName1=$date ."fei1" . '.' . $temp1[1];
		$newName2=$date ."fei2" . '.' . $temp2[2];
		$newName3=$date ."fei3" . '.' . $temp3[3];
		$newName4=$date ."fei4" . '.' . $temp4[4];
        ...
        if ((file_exists("upload/" . $newName1)) && (file_exists("upload/" . $newName2)) &&(file_exists("upload/" . $newName3)) && (file_exists("upload/" . $newName3)))
        {
...
cre_time,type,URL1,URL2,URL3,URL4,Details) values ('$time',' $zhonleixin','$photo1','$photo2','$photo3','$photo4','$jiesou')";
						...
						if($que){
							echo "<script>alert('添加成功');location.href='http://localhost:8080/lena/xindefu/xindefu-du-h-du/member-fenlei-feibiao.php'</script>";
						}else{
