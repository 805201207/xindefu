...
$sql = "SELECT * FROM xinxibiao LIMIT ".($page-1)*$PageSize .",$PageSize";
if(!$sql){
    echo "取出不成功";
};
//把sql语句传送到数据库
...
echo "<tr><td>ID</td><td>时间</td><td>建议</td><td>姓名</td><td>性别</td><td>手机号码</td><td>电话号码</td><td>传真</td><td>地址</td><td>编辑</td></tr>";
while($row = mysql_fetch_assoc($result)){
    ...
    echo "<td>{$row['Name']}</td>";
    echo "<td>{$row['Sex']}</td>";
    echo "<td>{$row['sjhao']}</td>";
    echo "<td>{$row['dhhao']}</td>";
    echo "<td>{$row['chuanzhen']}</td>";
    echo "<td>{$row['Address']}</td>";
    echo "<td>";
    ?>
    <a href="spyp-delete.php?id=<?php echo $row['id']?>">删除</a>
...
//获取数据总数
$to_sql="SELECT COUNT(*)FROM xinxibiao";
