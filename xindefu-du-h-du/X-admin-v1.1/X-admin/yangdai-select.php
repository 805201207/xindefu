...
$sql="select id from yangdai order by id desc";
$que=mysqli_query($conn,$sql);
$row=mysqli_fetch_array($que);
$id=$row["id"];//最大的id值
 for($i=1;$i<=$id;$i++){

 	$sql="select id,cre_time,type,Details from yangdai where id=$i";
	$que=mysqli_query($conn,$sql);
	$row=mysqli_fetch_array($que);
	 $ids=$row["id"];
	 $time=$row["cre_time"];
...
if($ids){
		...
		echo "<li><a href='feibiao-delete.php?id=$row[id]'>删除</a></li>";
		echo "</ul>";
